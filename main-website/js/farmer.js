...
window.switchFarmerTab = (btnId, contentId) => {
  ...
  currentFarmerTab = FARMER_TABS.find(t => t.btn === btnId)?.id || 'tabFarmerRegisterCrop';

  if (btnId === 'btnTabFarmerEnrolled' && typeof loadEnrolledCrops === 'function') loadEnrolledCrops();
  if (btnId === 'btnTabFarmerProfile' && typeof loadFarmerProfile === 'function') loadFarmerProfile();
  if (btnId === 'btnTabFarmerSell' && typeof loadNearbyBuyers === 'function') loadNearbyBuyers();
  if (btnId === 'btnTabFarmerMessages' && typeof loadConversations === 'function') loadConversations();
  if (btnId === 'btnTabFarmerPrice') {
    if (window.initGovernmentMandiPrices) {
      window.initGovernmentMandiPrices('farmer');
    } else if (typeof loadDefaultDailyMandiPrices === 'function') {
