...
window.switchBuyerTab = (btnId, contentId) => {
  ...
  currentBuyerTab = BUYER_TABS.find(t => t.btn === btnId)?.id || 'tabBuyerMarket';

  if (btnId === 'btnTabBuyerMarket' && typeof loadBuyerProfile === 'function') loadBuyerProfile();
  if (btnId === 'btnTabBuyerMyReqs' && typeof loadBuyerRequirements === 'function') loadBuyerRequirements();
  if (btnId === 'btnTabBuyerFindFarmers' && typeof loadAvailableFarmers === 'function') loadAvailableFarmers();
  if (btnId === 'btnTabBuyerMessages' && typeof loadConversations === 'function') loadConversations();
  if (btnId === 'btnTabBuyerPrice') {
    if (window.initGovernmentMandiPrices) {
      window.initGovernmentMandiPrices('buyer');
    } else if (typeof loadBuyerDailyMandiPrices === 'function') {
