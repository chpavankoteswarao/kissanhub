...
const initCommunicationSocket = () => {
  ...
  const socketTarget = window.BACKEND_ORIGIN || (window.KISSAN_CONFIG && window.KISSAN_CONFIG.BACKEND_URL ? window.KISSAN_CONFIG.BACKEND_URL.replace(/\/+$/, '') : undefined);
  const socket = socketTarget ? io(socketTarget, {
    auth: {
      userId: session.id,
